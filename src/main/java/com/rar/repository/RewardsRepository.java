...
public interface RewardsRepository extends CrudRepository<Rewards, Long> {
   ...
   List<Rewards> managerApprovalRewards();
    ...
    @Query(value = "select * from rewards  where reward_id not in(select distinct reward_id from nominations where hr_selected=true)  order by end_date asc ",nativeQuery = true)
